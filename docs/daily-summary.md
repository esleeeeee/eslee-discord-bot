...
- 평상시에는 하루 전체 대화를 단 한 번의 API 요청으로 요약한다. 한 요청의 프롬프트 상한
  (기본 200,000자)을 넘는 날만 시간순 청크 요약 후 최종 통합한다.
...
- Gemini `503`, timeout, network 오류는 한 요청당 최대 2회만 시도하고, 그 시도도 같은 예산에서
  차감한다. 운영 client의 SDK retry는 1회로 고정해 SDK와 애플리케이션 retry가 중첩되지 않는다.
