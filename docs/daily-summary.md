...
- 평상시에는 하루 전체 대화를 단 한 번의 API 요청으로 요약한다. 대화 원문이 한 요청에
  담을 수 있는 분량(기본 약 196,000자)을 넘는 날만 시간순 청크 요약 후 최종 통합한다.
- 성공한 청크 요약과 최종 통합 결과는 `daily_reports.ai_state_json`에 checkpoint로 저장한다.
  재시도나 프로세스 재시작 후에는 완료된 청크를 다시 호출하지 않고, 최종 통합만 실패했다면
  최종 통합만 다시 호출한다. Discord 게시만 실패한 경우에는 Gemini를 전혀 호출하지 않는다.
- 대상 날짜의 메시지가 수정·삭제돼 청크 내용이 달라지면 해당 청크의 checkpoint만 무효화한다.
...
현재 집계 구간의 시작인 가장 최근 06:00부터 현재까지 소스 채널을 읽고, 실시간 수집과 같은
필터를 적용한다.
...
리포트 첫 Embed의 하단에는 `전날 06시부터 오늘 06시까지의 기록입니다.`가 작은 footer로
표시된다. `/하루요약 오늘` 미리보기에는 아직 집계 중인 구간이라는 별도 footer를 표시한다.

## 명령어

- `/하루요약 상태`: 설정과 현재 06:00 기준 집계 구간의 저장량, 최근 리포트 상태 표시
- `/하루요약 오늘`: 가장 최근 06:00부터 현재까지 미리보기 생성
- `/하루요약 어제`: 직전 완료 구간(전날 06:00~당일 06:00)의 최종 리포트가 있으면 Gemini를
  다시 호출하지 않고 기존 Discord 리포트를 새 메시지로 복사해 게시. 저장된 최종 리포트가 없거나
  원본 리포트 메시지가 삭제된 경우에만 요약을 새로 생성
- `/하루요약 연결확인`: API 키 존재와 모델명을 표시하고 Gemini에 최소 1회 요청을 보내 인증·모델
  접근·텍스트 응답을 확인. API 키 값은 표시하지 않고 DB와 리포트를 변경하지 않음
- `오늘 재생성:true`: 이미 있는 오늘 미리보기를 다시 생성하고 기존 미리보기 메시지를 갱신

모든 `/하루요약` 명령은 기존 관리 권한 helper를 사용하며, 해당 서버의 소유자 또는 관리자만
실행할 수 있다. 명령어의 진행·성공·실패·중복·권한 거부 응답은 모두 실행자에게만 ephemeral로
보인다. 다른 서버에서 실행하면 해당 서버는 미지원임을 ephemeral로 안내한다. 완료된 일일 리포트
본문만 `DAILY_SUMMARY_REPORT_CHANNEL_ID`에 기존대로 공개 게시한다.
...
- 무료 등급 한도는 RPM 5, TPM 250K, RPD 20이다. Pacific 기준 하루 quota 창은 한국 시간
  16~17시에 초기화되므로, 초기화 이후의 보충 실행과 다음 날 06:02 자동 실행이 같은 quota 창을
  공유한다. 따라서 리포트 한 건이 쓸 수 있는 호출 수는 RPD보다 충분히 낮아야 한다.
- 생성 시작 전에 청크 수와 최종 통합을 포함한 예상 호출 수를 계산한다. 예상 호출 수가 남은 예산을
  넘으면 첫 요청도 보내지 않고 실패로 남긴다.
- 호출 예산은 **Pacific quota 구간별**로 센다. `daily_reports.ai_request_count`는 현재 구간에서
  쓴 횟수이고 `ai_quota_window`가 그 구간의 Pacific 날짜다. 구간이 바뀌면 카운터는 0부터
  다시 시작하므로, 한 구간에서 예산을 다 써도 리포트가 영구히 막히지 않는다.
- 자동 실행은 구간당 최대 8회, 수동 명령까지 합친 상한은 12회이며 자동·수동·보충 실행이 같은
  카운터를 쓴다. 상한에 도달하면 다음 quota 초기화까지 대기한다.
- `ai_request_total`은 리포트의 누적 호출 수로 감사용이며 구간이 바뀌어도 초기화하지 않는다.
- 리포트가 `completed`/`skipped`가 되면 checkpoint와 cooldown만 비우고, 그 구간에서 이미 쓴
  호출 수는 그대로 남겨 같은 구간의 상한이 계속 지켜지도록 한다.
- Gemini `503`, timeout, network 오류는 한 요청당 최대 2회만 시도하고, 그 시도도 같은 예산에서
  차감한다. 운영 client의 SDK 자체 재시도는 꺼 두어(총 1회 시도) SDK와 애플리케이션 retry가 중첩되지 않는다.
...
- 일일 RPD quota는 Pacific 자정에 초기화되므로 다음 자정에서 5분 지난 시각까지 대기한 뒤,
  ...
  달라진다. 이 대기는 같은 guild의 **모든** 날짜에 적용해, 초기화 전에는 다른 날짜의 자동 실행도
  요청을 보내지 않는다.
- 호출 사이에는 분당 요청 수와 예상 토큰 수를 함께 보는 pacing을 적용해 RPM·TPM을 넘지 않는다.
- scheduler는 `completed`/`skipped`만 그날의 종료 상태로 본다. 예산 소진은 종료가 아니라 다음
  quota 초기화 시각까지의 대기이므로, 같은 날 오후 초기화 이후 보충 실행이 가능하다.
