...
- 평상시에는 하루 전체 대화를 단 한 번의 API 요청으로 요약한다. 한 요청의 프롬프트 상한
  (기본 100,000자)을 넘는 날만 시간순 청크 요약 후 최종 통합한다.
...
- 날짜별 Gemini 호출 수는 `daily_reports.ai_request_count`에 요청 직전마다 저장한다. 자동 실행은
  리포트당 최대 8회, 수동 명령까지 합친 상한은 12회이며 자동·수동·보충 실행이 같은 카운터를 쓴다.
  리포트가 `completed`가 되면 카운터와 checkpoint를 비운다.
