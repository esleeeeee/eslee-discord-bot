...
Gemini 무료 등급 한도(RPM 5, TPM 250K, RPD 20)를 넘지 않도록 리포트 한 건이 쓸 수 있는 호출 수를 제한합니다. 생성 전에 예상 청크 수와 최종 통합을 포함한 호출 수를 계산해 남은 예산을 넘으면 첫 요청도 보내지 않습니다. 날짜별 호출 수는 요청 직전마다 DB에 저장하며 자동 실행은 리포트당 최대 8회, 수동 명령까지 합쳐도 12회를 넘지 않고 자동·수동·보충 실행이 같은 카운터를 공유합니다. 성공한 청크 요약과 최종 통합 결과는 checkpoint로 저장해 재시도나 재시작 후에도 다시 호출하지 않습니다. `429`는 같은 실행에서 반복 호출하지 않고, 분당 요청·토큰 제한은 최소 10분, 종류를 확인할 수 없는 quota 오류는 1시간 대기하며, 일일 quota 소진은 Pacific 자정 초기화 전까지 모든 날짜의 자동 호출을 멈춘 뒤 보충합니다. `503`·timeout·network 오류는 한 요청당 최대 2회만 시도하고 그 시도도 같은 예산에서 차감하며, SDK 자체 retry는 끕니다.
