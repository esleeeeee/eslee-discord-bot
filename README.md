...
Gemini `429`는 같은 실행에서 반복 호출하지 않습니다. 분당 요청·토큰 제한은 최소 10분, 종류를 확인할 수 없는 quota 오류는 1시간 동안 대기하고, 일일 quota 소진은 Pacific 자정의 초기화 이후 자동 보충합니다. `503`·timeout·network 오류만 한 실행에서 최대 3회 exponential backoff하며, SDK 자체 retry는 끄고 날짜별 자동 AI 호출은 최대 24회로 제한합니다.
