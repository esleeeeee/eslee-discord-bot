...
같은 봇을 여러 서버에 초대해도 기능마다 적용 범위가 다릅니다.

| 기능 | 적용 범위 | 설정 방법 |
| --- | --- | --- |
| 공지 리마인드 | **서버마다 독립** | 각 서버에서 명령으로 등록 |
| 금지어 | **서버마다 독립** | 각 서버에서 명령으로 등록 |
| 관리 로그 채널 | **서버마다 독립** | 각 서버에서 `/설정 로그채널` |
| 위반 기록 | **서버마다 독립** | 자동 |
| 하루 대화 요약 | **운영자가 지정한 한 서버·한 채널에서만** | `.env`의 `DAILY_SUMMARY_*` |
| OneKey 연동 API | **봇 프로세스 전체에 하나** | `.env`의 `ONEKEY_*`, `PORT` |

공지·금지어·설정·위반 기록은 서버 ID 기준으로 분리되어 저장되므로 한 서버의
데이터가 다른 서버에 보이지 않습니다.

하루 대화 요약만 예외입니다. `.env`에 적은 서버에서만 동작하고, 다른 서버에서
`/하루요약`을 실행하면 "이 서버는 하루요약 기능의 대상 서버가 아닙니다"라는
안내만 나옵니다. 서버마다 다른 요약 설정을 두는 기능은 없습니다.

OneKey 연동 API는 Discord 서버 설정이 아니라 봇을 실행하는 운영자의 환경변수로
켜집니다. 자세한 내용은 [OneKey 연동 API](#7-onekey-연동-api-선택)를 보세요.
