...
개인 Discord 서버에서 운영하는 관리 봇입니다. 공개 설치용 봇이나 초대 링크는 제공하지 않습니다.
...
일반 채팅 메시지에는 Discord의 ephemeral 응답을 사용할 수 없습니다. 금지어 위반 경고는 DM을 사용하지 않고, 모든 사용자에게 원래 채널에서 멘션과 함께 약 5초 동안만 표시한 뒤 자동 삭제합니다.
...
일일 요약은 환경변수로 지정한 한 서버의 한 텍스트 채널에만 선택적으로 적용됩니다. 봇의 공지·금지어·서버 설정 데이터는 `guild_id`별로 분리됩니다. 지정 채널에서는 사람이 작성한 텍스트만 수집하며 봇, Webhook, 시스템 메시지, 빈 메시지는 제외합니다.
...
매일 06:02에 전날 06:00부터 당일 06:00까지의 대화를 집계해 최소 메시지·참여자 조건을 만족하면 Gemini로 전체 요약과 사용자별 요약을 생성합니다. 06:02 정각을 놓치거나 봇이 그 이후에 재시작해도 완료된 리포트가 없으면 다음 scheduler tick에서 자동으로 보충 실행합니다. 첫 실행에서 일시적인 DB·Gemini·Discord 오류가 발생해도 scheduler 전체가 종료되지 않고 미완료 리포트를 다시 시도하며, 자동 실행과 수동 명령이 겹치면 날짜별 잠금과 DB 상태 확인으로 중복 Gemini 호출과 중복 게시를 막습니다. 리포트 하단에는 집계 구간을 작게 표시합니다. `/하루요약 어제`는 완성된 리포트가 있으면 Gemini를 다시 호출하지 않고 기존 Discord 리포트를 새 메시지로 복사하며, 원본이 없을 때만 새로 생성합니다. 완료된 리포트 본문은 지정된 리포트 채널에 공개 게시하지만 `/하루요약 상태`, `/하루요약 오늘`, `/하루요약 어제`, `/하루요약 연결확인`의 진행·성공·실패 응답은 실행한 관리자에게만 보입니다. `연결확인`은 API 키 값이나 대화를 공개하지 않고 최소 요청 한 번으로 인증·모델 접근·응답 수신 여부만 확인합니다.

Gemini 무료 등급 한도(RPM 5, TPM 250K, RPD 20)를 넘지 않도록 리포트 한 건이 쓸 수 있는 호출 수를 제한합니다. 생성 전에 예상 청크 수와 최종 통합을 포함한 호출 수를 계산해 남은 예산을 넘으면 첫 요청도 보내지 않습니다. 호출 예산은 Pacific quota 구간별로 세며, 구간이 바뀌면 카운터가 0부터 다시 시작하므로 한 구간의 예산 소진이 리포트를 영구히 막지 않습니다. 요청 직전마다 DB에 저장하고 자동 실행은 구간당 최대 8회, 수동 명령까지 합쳐도 12회를 넘지 않으며 자동·수동·보충 실행이 같은 카운터를 공유합니다. 누적 호출 수는 감사용으로 따로 보존합니다. 성공한 청크 요약과 최종 통합 결과는 checkpoint로 저장해 재시도나 재시작 후에도 다시 호출하지 않습니다. `429`는 같은 실행에서 반복 호출하지 않고, 분당 요청·토큰 제한은 최소 10분, 종류를 확인할 수 없는 quota 오류는 1시간 대기하며, 일일 quota 소진은 Pacific 자정 초기화 전까지 모든 날짜의 자동 호출을 멈춘 뒤 보충합니다. `503`·timeout·network 오류는 한 요청당 최대 2회만 시도하고 그 시도도 같은 예산에서 차감하며, SDK 자체 retry는 끕니다.
...
| `/하루요약 어제` | 설정 서버의 소유자/관리자 | 기존 최종본을 AI 재호출 없이 새 메시지로 게시 |
...
# ONEKEY_API_TOKEN은 공백 없이 32자 이상인 새 무작위 secret이어야 합니다.
ONEKEY_DISCORD_USER_ID=
ONEKEY_API_TOKEN=
PORT=8080
...
Windows 로그인 때 로컬 봇을 자동 실행해야 하는 경우 `scripts/install_scheduled_task.ps1`로 예약 작업을 만들 수 있습니다. Northflank 운영과 같은 Discord 토큰을 동시에 실행하면 중복 동작하므로 이 스크립트는 로컬 전용 봇을 운영할 때만 사용합니다.
...
OneKey 설정을 추가하면 Discord Gateway 봇과 같은 프로세스에서 경량 HTTP 서버가 실행됩니다. `GET /health`는 인증 없이 프로세스와 Discord 준비 상태를 반환하고, `GET /api/voice-status`는 `Authorization: Bearer <ONEKEY_API_TOKEN>`이 있어야 `{"in_voice": true|false}` 하나만 반환합니다. 어느 서버·채널인지는 응답에 포함하지 않으며, 모든 응답에 `Cache-Control: no-store`와 `Vary: Authorization`을 붙여 중간 캐시가 저장하거나 공유하지 못하게 합니다. 봇이 참여하지 않은 서버와 DM·그룹 DM 통화는 감지 범위 밖입니다.
...
3. 같은 secret group 또는 서비스의 runtime variables에 `DISCORD_TOKEN`, `ONEKEY_DISCORD_USER_ID`, `ONEKEY_API_TOKEN`을 추가합니다. OneKey 두 값은 반드시 함께 설정하고 API token은 공백 없이 32자 이상인 새 무작위 secret으로 보관합니다. 길이가 짧거나 앞뒤 공백이 있으면 기동 시 설정 검증에서 거부됩니다. 필요하다면 `LOG_LEVEL`, `SCHEDULER_POLL_SECONDS`도 추가합니다. 프로덕션에서는 `DISCORD_DEV_GUILD_ID`를 만들지 않습니다. 이 값들은 build argument가 아니라 실행 컨테이너에만 전달되는 runtime 변수여야 합니다.
...
현재 테스트는 금지어 정규화·우회 표기·오탐 방지·다중 감지, 권한, 공지 콘텐츠 분류, Poll 남은 시간, 장시간 다운타임 보정, 리마인드 교체, 임시 채널 경고, 서버별 DB 격리, 일일 요약 수집·백필·스케줄·Gemini 오류 처리와 개인정보 정책을 검증합니다.
