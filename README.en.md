...
Gemini `429` responses are not retried repeatedly inside one run. RPM/TPM limits use at least a ten-minute cooldown, unknown quota errors wait one hour, and daily quota exhaustion catches up after the next Pacific-midnight reset. Only `503`, timeout, and network failures retain up to three application-level exponential-backoff attempts. SDK retries are explicitly disabled, and automatic AI traffic is capped at 24 requests per report date.
